.head 0 +  Application Description: Objetivo: Criada para gerenciar mensagem que serão exibidas no sistema de Lacres on - line
Autor: Djalma Júnior (djalmajr)
Data: 21/02/2013
.head 1 -  Outline Version - 4.0.28
.head 1 +  Design-time Settings
.data VIEWINFO
0000: 6F00000001000000 FFFF01000D004347 5458566965775374 6174650400800000
0020: 0000000000240100 002C000000020000 0003000000FFFFFF FFFFFFFFFFF8FFFF
...
0080: 6D4C616372654D65 6E736167656D
.enddata
...
.head 2 +  Resources
.head 2 +  Variables
.data RESOURCE 0 0 1 878039473
0000: 2200000013000000 0000000000000000 02000001000000E4 020000D019000000
0020: 01FE00FF03
.enddata
...
.head 1 +  Form Window: frmLacreMensagem
.head 2 -  Class: cQCK1Form
.head 2 -  Property Template:
.head 2 -  Class DLL Name:
.head 2 -  Title: Cadastro de Mensagens para Sistema de Lacres
.head 2 -  Icon File:
...
.head 6 -  Set _sTextoStatusBar = 'Código da Mensagem'
.head 6 -  Set _sNomeParaMsgs = 'Mensagem'
...
.head 6 -  !
.head 6 -  Set _sConsultaFrmOpcoesColunaOuParam = '@TituloMensagem'
.head 6 -  Set _sConsultaFrmOpcoesCampo = 'exec dbvcen..LacreMensagemSequencialTituloS  @Sequencial = :frmLacreMensagem.dfnMensagem'
.head 6 -  Set _sConsultaFrmOpcoesSQLOuSP = 'SP'
.head 6 -  Call SalSendClassMessage( SAM_Create, wParam, lParam )
.head 3 +  Data Field: dfsTituloMensagem
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class: cQCK1DataField
.head 4 -  Property Template:
...
.head 6 -  Set _bPermiteNulos = TRUE
.head 6 -  Set _sCamposResponsaveis[0] = 'dfnMensagem'
.head 6 -  Set _sConsultaDescrição = 'exec dbvcen..LacreMensagemTituloS @Sequencial = :frmLacreMensagem.dfnMensagem'
.head 6 -  Call SalSendClassMessage( SAM_Create, wParam, lParam )
.head 3 +  Pushbutton: pbCredenciados
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class: cQCK2PbComum
.head 4 -  Property Template:
.head 4 -  Class DLL Name:
.head 4 -  Title: Credenciados
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set _sTextoStatusBar = 'Chama tela para associar os credenciados que irão visualizar a mensagem'
.head 6 -  !
.head 6 -  Set _nPermissao = EF_RegistroExistente
.head 6 -  !
.head 6 -  Call SalSendClassMessage( SAM_Create, wParam, lParam )
.head 5 +  On SAM_Click
.head 6 +  If hWndCredenciados != hWndNULL
.head 7 -  Call SalSendMsg( hWndCredenciados, MSG_Sai, 0, 0 )
.head 6 -  Set hWndCredenciados = SalCreateWindow( frmLacreMensagemCredenciado, hWndMDI, hWndItem, hWndCredenciados, dfnMensagem, pbAnterior, pbProximo)
...
.head 6 -  Set dfnMensagem = 0
.head 6 -  Call SalDisableWindow(dfnMensagem)
.head 6 -  Call SalEnableWindow(dfsTituloMensagem)
.head 5 +  On SAM_Create
.head 6 -  Set _sTextoStatusBar = 'Habilita a tela para inserção de um novo registro'
...
.head 6 -  Call SalSendClassMessage( SAM_Create, wParam, lParam )
.head 5 +  On SAM_Click
.head 6 -  ! Set sTexto =  frmLacreMensagem.mlCorpoMensagem
.head 6 +  If SalIsNull(dfsTituloMensagem)
.head 7 -  Call SalMessageBeep( MB_IconExclamation )
.head 7 -  Call SalMessageBox( 'Favor informar informar o Título da Mensagem !', 'Atenção', MB_IconExclamation | MB_Ok )
.head 7 -  Return FALSE
.head 6 +  If SalIsNull(mlCorpoMensagem)
.head 7 -  Call SalMessageBeep( MB_IconExclamation )
.head 7 -  Call SalMessageBox( 'Favor informar informar o Corpo da Mensagem !', 'Atenção', MB_IconExclamation | MB_Ok )
.head 7 -  Return FALSE
.head 6 -  ! Set nStringLength = SalStrLength ( sTexto )
.head 6 -  ! Set nParte = 1
.head 6 +  ! While nStringLength > 0
.head 7 -  Set sCertidaoNarrativa[nParte] = SalStrMidX( sTexto, 0, 255 )
.head 7 -  Set sTexto = SalStrMidX( sTexto, 255, 510 )
.head 7 -  Set nTeste = SalStrLength ( sTexto )
.head 7 -  Set nStringLength = (nStringLength - 255)
.head 7 -  Set nParte = nParte + 1
.head 6 -  Call SalSendClassMessage( SAM_Click, wParam, lParam )
...
.head 6 -  Call SalSendClassMessage( SAM_Create, wParam, lParam )
.head 5 +  On SAM_Click
.head 6 -  Call SalEnableWindow(dfnMensagem)
.head 6 -  Call SalSendClassMessage( SAM_Click, wParam, lParam )
.head 3 +  ! cQCK1PushButton: pbExcluir
...
.head 3 +  Multiline Field: mlCorpoMensagem
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set _sTextoStatusBar = 'Corpo da Mensagem'
.head 6 -  Set _bVisivel = TRUE
.head 6 -  Set _bComponenteChave = FALSE
.head 6 -  Set _sNomeParaMsgs = 'Corpo da Mensagem'
...
.head 4 -  Text Color: Class Default
.head 4 -  Background Color: Class Default
.head 3 -  Background Text: Corpo da Mensagem:
.head 4 -  Resource Id: 24336
.head 4 -  Class Child Ref Key: 0
.head 4 -  Class ChildKey: 0
.head 4 -  Class: cQCK1BackgroundText
.head 4 -  Window Location and Size
...
.head 7 -  Call fMPSPrepareProc ( hSql, 'exec dbvcen..LacreMensagemEntradaS @Sequencial = :frmLacreMensagem.nSequencial',
':frmLacreMensagem.dfnMensagem,  
:frmLacreMensagem.dfsTituloMensagem,  
:frmLacreMensagem.mlCorpoMensagem')
.head 7 -  Call SqlExecute( hSql)
.head 7 -  Call SqlFetchNext( hSql, lParam )
.head 7 -  ! Call SqlCommit(hSql)
.head 6 -  ! Set frmLacreMensagem.mlCorpoMensagem = ''
.head 6 -  ! Set frmLacreMensagem.mlCorpoMensagem = sCertidaoNarrativa[1]||sCertidaoNarrativa[2]
...
.head 4 +  Message Actions
.head 5 +  On SAM_Create
.head 6 -  Set _sTextoStatusBar = 'Move para o próximo registro'
...
.head 3 -  Number: nSequencial
.head 3 -  Number: nSetor
.head 3 -  Number: nTeste
.head 3 -  Number: nParte
.head 3 -  Number: nStringLength
.head 3 -  Long String: sTexto
.head 3 -  Long String: sCertidaoNarrativa[*]
.head 3 -  Sql Handle: hSql
.head 3 -  Sql Handle: hSqlx
.head 3 -  Number: nPos
.head 3 -  Window Handle: hWndCredenciados
.head 2 +  Message Actions
.head 3 +  On SAM_Create
.head 4 -  Set _nEstadoInicialForm = EF_Vazio
.head 4 -  Set _sTextoStatusBar = 'Tela de Cadastro de Mensagens do Sistema de Lacres'
.head 4 -  !
.head 4 -  Set _sCriacaoConexao = 'Default'
.head 4 -  Set _sComandoPesquisa = 'exec dbvcen..LacreMensagemEntradaS @Sequencial = :frmLacreMensagem.dfnMensagem'
.head 4 -  !
.head 4 -  Set _sListaINTOPesquisa = ':frmLacreMensagem.dfnMensagem, 
		               :frmLacreMensagem.dfsTituloMensagem, 
		               :frmLacreMensagem.mlCorpoMensagem'
.head 4 -  !
.head 4 -  Set _bLimpaTelaAposInclusao = FALSE
.head 4 -  Set _sComandoInclusao = 'exec dbvcen..LacreMensagemEntradaI
		           @TituloMensagem 		= :frmLacreMensagem.dfsTituloMensagem, 
		           @CorpoMensagemFinal ="' || frmLacreMensagem.mlCorpoMensagem ||'"'
		           
.head 4 -  Set _sListaINTOInclusao = ':frmLacreMensagem.dfnMensagem'
.head 4 -  !
.head 4 -  Set _bLimpaTelaAposAtualizacao = FALSE
.head 4 -  Set _sComandoAtualizacao = 'exec dbvcen..LacreMensagemEntradaA 
			@Sequencial 		= :frmLacreMensagem.dfnMensagem,
			@TituloMensagem 		= :frmLacreMensagem.dfsTituloMensagem, 
		                  @CorpoMensagemFinal ="' || frmLacreMensagem.mlCorpoMensagem ||'"'
...
.head 4 -  Call SalEnableWindow(dfnMensagem)
.head 4 -  !
.head 4 +  If frmLacreMensagem.fObtemPermissao('frmLacreMensagem') = 1
.head 5 -  Call SalDisableWindow( pbArquivar )
.head 4 +  Else If frmLacreMensagem.fObtemPermissao('frmLacreMensagem') = 15
.head 5 -  Call SalEnableWindow( pbArquivar )
.head 3 +  On UDM_AposPesquisa
.head 4 -  ! Set frmLacreMensagem.mlCorpoMensagem = sCertidaoNarrativa[1]||sCertidaoNarrativa[2]
.head 4 -  ! Set nSequencial = frmLacreMensagem.dfnMensagem
.head 4 -  ! Call VisArrayDeleteItem(sCertidaoNarrativa,1,DT_String)
.head 4 -  ! Call VisArrayDeleteItem(sCertidaoNarrativa,2,DT_String)
.head 4 -  Call SalEnableWindow(dfsTituloMensagem)
.head 4 -  Call SalEnableWindow(mlCorpoMensagem)
.head 4 -  Call SalDisableWindow(dfnMensagem)
.head 4 -  Call SalMessageBeep( MB_IconExclamation )
.head 4 -  ! Call SalMessageBox( 'Arquivamento realizado com sucesso!', 'Atenção', MB_IconExclamation | MB_Ok )
.head 4 -  Return TRUE
.head 3 +  On UDM_AposInclusao
.head 4 -  ! Call VisArrayDeleteItem(sCertidaoNarrativa,1,DT_String)
.head 4 -  ! Call VisArrayDeleteItem(sCertidaoNarrativa,2,DT_String)
.head 4 -  Call SalDisableWindow(dfsTituloMensagem)
.head 4 -  Call SalDisableWindow(pbArquivar)
.head 4 -  Call SalDisableWindow(mlCorpoMensagem)
.head 4 -  Call SalMessageBeep( MB_IconExclamation )
.head 4 -  Call SalMessageBox( 'Arquivamento realizado com sucesso!', 'Atenção', MB_IconExclamation | MB_Ok )
.head 4 -  Return TRUE
...
.head 4 -  Call SalDisableWindow(pbArquivar)
.head 4 -  Call SalDisableWindow(mlCorpoMensagem)
.head 4 -  Call SalMessageBox( 'Atualização realizada com sucesso!', 'Atenção', MB_IconExclamation | MB_Ok )
.head 4 -  Return TRUE
.head 3 +  On UDM_AntesAtualizacao
.head 4 -  Set nPos = 1
.head 4 +  While (nPos >= 0)
.head 5 -  Set nPos = SalStrScan(frmLacreMensagem.mlCorpoMensagem,"'")
.head 5 +  If (nPos >= 0)
.head 6 -  Set frmLacreMensagem.mlCorpoMensagem = SalStrReplaceX(frmLacreMensagem.mlCorpoMensagem,nPos,1,'"')
.head 4 -  Set _sComandoAtualizacao = "exec dbvcen..LacreMensagemEntradaA 
			@Sequencial 		= :frmLacreMensagem.dfnMensagem,
			@TituloMensagem 		= :frmLacreMensagem.dfsTituloMensagem, 
		                  @CorpoMensagemFinal ='" || frmLacreMensagem.mlCorpoMensagem ||"'"
.head 4 -  Return SalSendClassMessage( UDM_AntesAtualizacao, wParam, lParam )
...
.head 4 +  If hWndCredenciados != hWndNULL
.head 5 -  Call SalSendMsg( hWndCredenciados, SAM_Close, wParam, lParam )
