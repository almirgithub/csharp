...
0020: 00000000008D0000 002C000000020000 0003000000FFFFFF FFFFFFFFFFF8FFFF
...
0080: 6D4C6F6752657374 726963616F536572 7669636F
.enddata
...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmLogRestricaoServico
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Registros de ocorrências (log)  no cadastro das restrições de serviço
		Icon File:
		...
		Description: --------------------------------------------------------------------------------------------------------
				Criação: Consulta de ocorrências (log) realizadas no cadastro de restricao de servico.
				Autor-Data: [jhqb] João Queiroz - 12/09/2012
				--------------------------------------------------------------------------------------------------------
		Named Menus
		Menu
		Tool Bar
...
.enddata
				...
				Message Actions
					...
					On SAM_KillFocus
						...
						! Call SalSendMsg( cmbCategoria, MSG_Habilita1, lParam, wParam )
...
.enddata
			...
			Data Field: dfsCod
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Sigla do usuário do sistema'
						Set _sNroCamposIgnorados = 'NENHUM'
						!
						Set _bVisivel = TRUE
						Set _bPermiteNulos = FALSE
			...
			Data Field: dfsNome
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Nome do operador'
						Set _sNroCamposIgnorados = 'NENHUM'
						!
						...
						!
						Set _bVisivel = TRUE
						Set _sNomeParaMsgs = 'Nome'
						Set _bÉDependente = TRUE
						Set _sCamposResponsaveis[0] = 'dfsCod'
						Set _sConsultaDescrição = 'exec UsuarioS_nome @Cod = :frmLogRestricaoServico.dfsCod'
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			Combo Box: cmbRestricao
				Class Child Ref Key: 0
				Class ChildKey: 0
				Class: cQCK1ComboBox
				Property Template:
...
.enddata
			...
			Combo Box: cmbTipoServico
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Tipo de Serviço'
						Set _sNomeParaMsgs = 'Categoria'
						Set _sTipoMsgNulo = 'PADRAO'
						Set _bVisivel = TRUE
						Set _bSempreHabilitado = TRUE
						Set _sRefresh = 'Create'
						Set _sNomeCampoCodigo = 'dfnTipoServico'
						Set _sConsultaBD = 'exec dbvcen..TipoServicoS_coddesc'
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					! On SAM_Click
						Call SalSendClassMessage( SAM_Click, wParam, lParam )
						! Set nCategoria = SalStrToNumber( dfsRestricao )
...
.enddata
				...
				Message Actions
					...
					On SAM_Click
						Call SalSendClassMessage( SAM_Click, wParam, lParam )
						Call SalSendMsg( tblLogRestricaoServico, MSG_PesquisaTabela, 0, 0 )
			Pushbutton: pbLimpar
.data INHERITPROPS
0000: 0100
.enddata
			...
			Pushbutton: pbSair
				...
				Message Actions
					On SAM_Create
						...
						Set _sTabela = ''
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			! ! ----- tabela de ocorrências (Log)
			! cQCK1BackgroundText: Usuário:
...
.end
				Message Actions 
					On SAM_Create
						Set _sTextoStatusBar = 'Nome do usuário'
						Set _sNroCamposIgnorados = 'NENHUM'
						!
						Set _bNuncaHabilitado = TRUE
						Set _bPermiteNulos = FALSE
						...
						Set _sNomeParaMsgs = 'Nome'
						Set _bÉDependente = TRUE
						Set _sCamposResponsaveis[0] = 'dfsCodUsu'
						Set _sConsultaDescrição = 'exec UsuarioS_nome @Cod = :frmLogRestricaoServico.dfsCodUsu'
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			Background Text: Data Inicial:
				...
				Text Color: Default
				Background Color: Default
			Child Table: tblLogRestricaoServico
				Class Child Ref Key: 0
				Class ChildKey: 0
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Registros de ocorrências relacionados a Restrição de Seviço'
						Set _sNroCamposIgnorados = 'NENHUM'
						!
						...
						Set _bSempreHabilitado = FALSE
						Set _sComandoPesquisa = 'Exec dbvcen..LogRestricaoServicoS
												@sOperador =:frmLogRestricaoServico.dfsCod,
												@dDataInicial =:frmLogRestricaoServico.dfdDataIni,
												@dDataFinal =:frmLogRestricaoServico.dfdDataFim,
												@sRestricao =:frmLogRestricaoServico.dfsRestricao,
												@nTipoServico = :frmLogRestricaoServico.dfnTipoServico
											'
						Set _sIntoPesquisa = '
								:frmLogRestricaoServico.tblLogRestricaoServico.colsOperador,
								:frmLogRestricaoServico.tblLogRestricaoServico.colsNomeOperador,
								:frmLogRestricaoServico.tblLogRestricaoServico.coldDataOperacao,
								:frmLogRestricaoServico.tblLogRestricaoServico.colsOperacao,
								:frmLogRestricaoServico.tblLogRestricaoServico.colnTipoServico,
								:frmLogRestricaoServico.tblLogRestricaoServico.colsDescTipoServico,
								:frmLogRestricaoServico.tblLogRestricaoServico.colnRestricao,
								:frmLogRestricaoServico.tblLogRestricaoServico.colsDescRest
								'
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					On UDM_AposPesquisa
						If NOT SalTblAnyRows( tblLogRestricaoServico, 0, 0 )
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'Nehum registro de Log encontrado!', 'Atenção', MB_IconExclamation | MB_Ok )
							! Call SalSendMsg( pbImprimir, MSG_Desabilita1, lParam, wParam )
							Call SalWaitCursor( FALSE )
							Call SalAbort( -1)
						Else
							! Call SalSendMsg( pbImprimir, MSG_Habilita1, lParam, wParam )
						Call SalSendMsg( pbPesquisar, MSG_Habilita1, lParam, wParam )
						! Call SalSendMsg( cmbCategoria, MSG_Habilita1, lParam, wParam )
						Call SalWaitCursor( FALSE )
						Return TRUE
		...
		Message Actions
			On SAM_Create
				Set _sCriacaoConexao = 'default'
				Call SalSendClassMessage( SAM_Create, wParam, lParam )
				Call SalCenterWindow( frmLogRestricaoServico )
			On SAM_CreateComplete
				Call SalSendMsg( pbPesquisar, MSG_Habilita1, lParam, wParam )
				! Call SalSendMsg( cmbCategoria, MSG_Habilita1, lParam, wParam )
			On SAM_Close
				! Call SalDestroyWindow( hWndJanela )
