...
.enddata
		...
		Description: --------------------------------------------------------------------------------------------------------
				Criação: Tela de cadastro de máquinas.
				Autor-Data: [fhv] Fernando Veras - 11/05/2016
				--------------------------------------------------------------------------------------------------------
		Named Menus
		Menu
		Tool Bar
		...
		Contents
			...
			Data Field: dfsNome
				...
				Message Actions
					On SAM_Create
						Set _sNomeParaMsgs = 'Nome da máquina'
						Set _bVisivel = TRUE
...
.enddata
		...
		Message Actions
			On SAM_Create
				...
				! ----------------------------------------Inclusão-------------------------------------
				Set _sComandoInclusao = 'dbvcen..MaquinaSetorI @SetorMaquina = :frmMaquina.pnSetor, @MaquinaNova = :frmMaquina.dfsNome'
				!
				Set _bLimpaTelaAposInclusao = TRUE
				!
				Set _sListaINTOInclusao = ''
				! ----------------------------------------Alteração-----------------------------------
				Set _sComandoAtualizacao = 'dbvcen..MaquinaSetorI @SetorMaquina = :frmMaquina.pnSetor, @MaquinaNova = :frmMaquina.dfsNome'
				!
				...
				Set _sTextoStatusBar = 'Tela de cadastro de Máquinas'
				Call SalSendClassMessage( SAM_Create, wParam, lParam )
				Call SalCenterWindow ( hWndForm )
				Call SalSetFocus( dfsNome )
			On UDM_AposAtualizacao
				Call SalMessageBox( 'Arquivamento realizado com Sucesso!', 'Atenção', MB_IconExclamation | MB_Ok )
				Call SalSendMsg( frmMaquinaSetor.lbMaquinaDisponivel, MSG_Refresh, 0,0 )
				Call SalSendMsg( frmMaquinaSetor.lbMaquinaAssociado, MSG_Refresh, 0,0 )
				Call SalDestroyWindow( frmMaquina )
			On MSG_Limpa
