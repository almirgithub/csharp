...
0080: 6D52656C52656C61 63616F5365727669 636F
.enddata
...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmRelRelacaoServico
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Relatório Relação de Serviço
		Icon File:
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _bVisivel = TRUE
						Set _sTextoStatusBar = 'Clique caso deseje enviar o relatório para a impressora'
						Set _sValorCampoFonte = 'E'
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _bVisivel = TRUE
						Set _sTextoStatusBar = 'Clique caso deseje enviar o relatório para a tela'
...
.enddata
				...
				Message Actions
					On SAM_Click
						! Mostra o relatório
						If dfsSaida='I'
							Call SalReportPrint( hWndForm, 'RelRelacaoServico.qrp', 'sGlobalSecretaria, sGlobalDetran, nGlobalSetor, sGlobalSetor, nCod, sDescricao, nValor, sTipoIE, sServicoAtivo, SqlUser',
											'Secretaria, Detran, Setor,  NomeSetor, Cod, Descricao, Valor, Tipo, ServicoAtivo, Usuario', 1,RPT_PrintAll,0,0, nErro )
						Else
							Set hWndReportView= SalCreateWindow( frmReportView, hWndForm )
							Call SalSetWindowText( hWndReportView, 'Relatório Relação de Serviços' )
							Call SalReportView( hWndForm, hWndReportView,'RelRelacaoServico.qrp', 'sGlobalSecretaria, sGlobalDetran, nGlobalSetor, sGlobalSetor, nCod, sDescricao, nValor, sTipoIE, sServicoAtivo, SqlUser',
											'Secretaria, Detran, Setor,  NomeSetor, Cod, Descricao, Valor, Tipo, ServicoAtivo, Usuario',  nErro )
					On SAM_Create
						Set _sTabela = ''
...
.enddata
		...
		Window Variables
			...
			Number: nValor
			String: sTipoIE
			String: sServicoAtivo
		Message Actions
			On SAM_Create
				Call SalSendClassMessage( SAM_Create, wParam, lParam )
				Call SalCenterWindow( hWndForm )
			On SAM_ReportStart
				Call SalReportDlgOptions( SalNumberToWindowHandle( wParam ), 'Impressão', 'Imprimindo', 'Relatorio Relação Servicos', 'Relatorio Relação Servicos' )
				Return TRUE
			On SAM_ReportFetchInit
				Call fMPSPrepareProc( hSql, 	'exec dbrelat..TipoServicoR @Situacao = :frmRelRelacaoServico.dfsSituacao, @TipoIE =:frmRelRelacaoServico.dfsTipo', 
									':nCod, :sDescricao, :nValor, :sTipoIE, :sServicoAtivo' )
			...
			On SAM_ReportFetchNext
				...
				! Call SalFmtFieldToStr( dfsCGC, sCGC, TRUE )
				If nErro != FETCH_EOF
					Return 1
				Else
					Return 0
