...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmRelMultasNPagas
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Relatório de Multas Não Pagas
		Icon File:
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _sConsultaFrmOpcoesCampo = 'exec dbinfracao..OrgaoAutuanteS_desc_Estan @Setor=:frmRelMultasNPagas.nSetorUsuario'
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _sNroCamposIgnorados = 'NENHUM'
						Set _bVisivel = TRUE
						Set _sNomeParaMsgs = 'Agente/Equipamentos'
						Set _sConsultaFrmOpcoesSQLOuSP = 'SP'
						Set _sConsultaFrmOpcoesCampo = 'exec dbinfracao..AgenteEquipS_desc_Estan @OrgaoAutuante = :frmRelMultasNPagas.dfnOrgao, @Setor=:frmRelMultasNPagas.nSetorUsuario'
						Set _bComponenteChave = TRUE
						...
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					On UDM_ValidaCampo
						Return dfnAgenteEquip <= 2147483647
			Data Field: dfsAgenteEquip
.data INHERITPROPS
0000: 0100
.enddata
.data CLASSPROPS
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _sConsultaDescrição = 'exec dbinfracao..AgenteEquipS_desc_sint @Cod = :frmRelMultasNPagas.dfnAgenteEquip,
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _bNuncaHabilitado = TRUE
						Set _bÉCampoDescrição = TRUE
						Set _sConsultaDescrição ='exec OrgaoAutuanteS_desc_sintRest @Cod = :frmRelMultasNPagas.dfnOrgao, @SetorReduzido=:frmRelMultasNPagas.nSetorReduzido'
						Set _bÉDependente = TRUE
...
.enddata
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Clique para que o relatório seje impresso'
						Set _bVisivel = TRUE
						Set _bSempreHabilitado = TRUE
						...
						Call SalHideWindow( dfTexto )
						Call SalHideWindow( dfArquivo )
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					On SAM_Click
...
.enddata
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Clique para que o relatório saia na tela'
						Set _bVisivel = TRUE
						Set _bSempreHabilitado = TRUE
						! Variáveis para Check Box
			...
			Radio Button: rbArquivo
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Clique para que o relatório seja gerado em arquivo'
						Set _bVisivel = TRUE
						Set _bSempreHabilitado = TRUE
						Set _sTipoMsgNulo = 'PADRAO'
						! Variáveis para Check Box
					...
					On SAM_Click
						Call SalSendClassMessage( SAM_Click, wParam, lParam )
						Call SalDisableWindow( pbImpressora )
						Call SalShowWindow( dfArquivo )
						Call SalShowWindow( dfTexto )
						Set dfTexto = 'Informe o Nome do Arquivo:'
			Pushbutton: pbImprimir
				...
				Message Actions
					...
					On SAM_Click
						! Data Obrigatórias
						If dfnOrgao = NUMBER_Null
							Call SalMessageBeep( MB_IconExclamation )
							Call SalMessageBox( 'Informe um Órgão Autuante', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfnOrgao)
							Return FALSE
						If dfnAgenteEquip = NUMBER_Null
							Call SalMessageBeep( MB_IconExclamation )
							Call SalMessageBox( 'Informe um Agente/Equipamento', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfnAgenteEquip)
							Return FALSE
						If SalIsNull( dfdInicial)
							Call SalMessageBeep( MB_IconExclamation )
							Call SalMessageBox( 'A data de inicial é obrigatória', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfdInicial)
							Return FALSE
						If SalIsNull( dfdFinal)
							Call SalMessageBeep( MB_IconExclamation )
							Call SalMessageBox( 'A data de final é obrigatória', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfdFinal)
							Return FALSE
						If dfdInicial > dfdFinal
							Call SalMessageBeep( MB_IconExclamation )
							Call SalMessageBox( 'A data de criação inicial deve ser menor do que a data de criação final', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfdInicial)
							Return FALSE
						If hWndReport != hWndNULL
							Call SalSendMsg( hWndReport, SAM_Close, 0,0)
						If rbImpressora
							Call SalReportPrint( hWndForm,'MulNaoPg.qrp',
									'sGlobalSecretaria, sGlobalDetran, nGlobalSetor, sGlobalSetor, dCorrente, SqlUser,dDataImplantacao,
									dfsOrgao, sSerie, sAuto, sInfracao, dfdInicial, dfdFinal,nValorEmUFIR,nValorDaUFIR,nValorEmReal,nValorComDesconto,dfsAgenteEquip,nProtocolo,sAssunto,sDecisao',
									'Secretaria, Detran, Setor, DesSetor, DataCorrente, Usuario,DataImplantacao, OrgaoAutuante, Serie, Auto,
									Infracao, DataI, DataF,ValorEmUfir,ValorDaUfir,ValorEmReal,ValorComDesconto,AgenteEquip,Protocolo,Assunto,Decisao', 1,RPT_PrintAll,0,0, nErro )
						Else If rbTela
							Set hWndReport = SalCreateWindow( frmReportView, hWndForm )
							Call SalSetWindowText( hWndReport, 'Relatório de Notificações de Multa' )
							Call SalReportView( hWndForm, hWndReport, 'MulNaoPg.qrp',
									'sGlobalSecretaria, sGlobalDetran, nGlobalSetor, sGlobalSetor, dCorrente, SqlUser,dDataImplantacao,
									dfsOrgao, sSerie, sAuto, sInfracao, dfdInicial, dfdFinal,nValorEmUFIR,nValorDaUFIR,nValorEmReal,nValorComDesconto,dfsAgenteEquip,nProtocolo,sAssunto,sDecisao',
									'Secretaria, Detran, Setor, DesSetor, DataCorrente, Usuario,DataImplantacao, OrgaoAutuante, Serie, Auto,
									Infracao, DataI, DataF,ValorEmUfir,ValorDaUfir,ValorEmReal,ValorComDesconto,AgenteEquip,Protocolo,Assunto,Decisao', nErro)
						Else
							If dfArquivo = ''
								Call SalMessageBox( 'O Nome do Arquivo Não Foi Informado', 'Atenção', MB_IconExclamation )
								Return FALSE
							Else
								Call fMontaArquivo()
...
.enddata
			...
			Data Field: dfTexto
				...
				Message Actions
					On SAM_Create
						...
						Set _bVisivel = TRUE
						Set _sNomeParaMsgs = ''
						Set _sConsultaFrmOpcoesSQLOuSP = ''
						Set _sConsultaFrmOpcoesCampo = ''
						Set _bNuncaHabilitado = TRUE
						Set _bSempreHabilitado = FALSE
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			Data Field: dfArquivo
				...
				Message Actions
					On SAM_Create
						...
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					! On PAM_SetaFoco
						  Call SalSendClassMessage( PAM_SetaFoco, wParam, lParam )
					! On PAM_SetaFoco
			Picture: picSemaforo
		...
		Functions
			Function: fMontaArquivo
				...
				Local variables
					...
					String: sSequencial
				Actions
					! Set sArquivo = 'C:\\'||SalNumberToStrX(SalDateYear( SalDateCurrent(  )),0)||
					...
					Call SalHideWindow( dfArquivo )
					Call SalHideWindow( dfTexto )
					Call SalShowWindow( dfMensagem )
					Set sArquivo = 'C:\\'||dfArquivo||"."||SalStrMidX( SalNumberToStrX(dfnOrgao,0), 0, 3 )
					Call SalWaitCursor( TRUE )
					Set dfMensagem = 'GERANDO O ARQUIVO: '||sArquivo
					! Call SalModalDialog( dlgEsperaGeracao, frmRelMultasPagas )
					! Set dlgEsperaGeracao.dfArquivo = sArquivo
					! Call fMPSPrepareProc( hSql, 'exec RelMultasNPagas @OrgaoAutuante = :frmRelMultasPagas.dfnOrgao,
							@DataI = :frmRelMultasPagas.dfdInicial,  @DataF = :frmRelMultasPagas.dfdFinal,@Saida="A"',
							
					Call fMPSPrepareProc( hSql, 'exec RelMultasNaoPagas @OrgaoAutuante = :frmRelMultasNPagas.dfnOrgao,
							@DataI = :frmRelMultasNPagas.dfdInicial,  @DataF = :frmRelMultasNPagas.dfdFinal,@Saida="A"',
							':sTexto')
					If NOT SqlExecute( hSql )
						Call SalMessageBeep( 0 )
						Call SalMessageBox( 'Ocorreu um erro durante a execução. Chame o Analista Responsável.', 'Atenção', MB_Ok | MB_IconExclamation )
						Return FALSE
					!
					Set bok =  SalFileOpen( hArquivo, sArquivo,OF_Write|OF_Create )
					If not bok
						Call SalMessageBeep( 0 )
						Call SalMessageBox( 'Ocorreu um erro durante a abertura do arquivo txt. Chame o Analista Responsável.', 'Atenção', MB_Ok | MB_IconExclamation )
					Set nRegistros = 0
					Call SqlFetchNext( hSql, nProximo )
					!
					Loop
						If nProximo = FETCH_EOF
							Break
						Set nRegistros = nRegistros + 1
						Set sSequencial = SalStrRepeatX( '0', 8-SalStrLength(SalNumberToStrX( nRegistros, 0 )))||SalNumberToStrX( nRegistros, 0 )
						Call SalFilePutStr( hArquivo, sSequencial||sTexto )
						Call SqlFetchNext( hSql, nProximo )
					Call SalFileClose( hArquivo )
					Call SalMessageBox(  'O ARQUIVO: '||sArquivo ||' FOI GERADO COM SUCESSO', 'ATENÇÃO', MB_IconExclamation )
					Set dfMensagem = ''
					If SalMessageBox( 'Deseja visualizar o layout do arquivo gerado?', 'Atenção',MB_YesNo )=IDYES
						Set hWndReport = SalCreateWindow( frmReportView, hWndForm )
						Call SalReportView( frmRelMultasNPagas, hWndReport, 'MulNaoP.qrp',
					...
					Call SalSendMsgToChildren( frmRelMultasNPagas, MSG_Limpa,wParam,lParam )
					Call SalWaitCursor( FALSE )
					Return TRUE
		Window Parameters
		Window Variables
			! Variáveis para a impressão
			Date/Time: dCorrente
			String: sTipoArquivo
			Date/Time: dCriacao
			String: sArquivo
			Long String: lsDiretorio
			Date/Time: dPeriodoIni
			Date/Time: dPeriodoFin
			Number: nRegistros
			Number: nValor
			! Adilson
			Number: nValorEmUFIR
			Number: nValorDaUFIR
			Number: nValorEmReal
			Number: nValorComDesconto
			! adilson Fim
			String: sAuto
			String: sInfracao
			String: sSerie
			String: sOrgaoAutuante
			Date/Time: dDataImplantacao
			! Para mais de um result set
			Boolean: bAux
			Boolean: bAbortaImpressao
			String: sStored
			! Variáveis 'padrão' de relatório
			Number: nErro
			String: sDevice
			String: sDriver
			String: sPorta
		...
		Message Actions
			! Observações
				  Autor: Roberto T. - 09/02/2000
			On SAM_Create
				Set _sTextoStatusBar = 'Tela de relatório de multas implantadas e não pagas'
				...
				Set nSetorReduzido=nGlobalSetor
			On SAM_Destroy
				If hWndReport != hWndNULL
					Call SalSendMsg( hWndReport, SAM_Close, 0,0)
			...
			On SAM_ReportFetchInit
				Call SqlImmediate( 'select getdate() into :frmRelMultasNPagas.dCorrente' )
				Call fMPSPrepareProc( hSql, 'exec dbinfracao..RelMultasNaoPagas @OrgaoAutuante = :frmRelMultasNPagas.dfnOrgao,
