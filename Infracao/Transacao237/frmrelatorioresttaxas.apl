...
0080: 6D52656C61746F72 696F526573745461 786173
.enddata
...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmRelatorioRestTaxas
.data INHERITPROPS
0000: 0100
.enddata
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Relatório de Restituições de Taxas
		Icon File:
		...
		Description: Relatório de Restituicoes de Taxas
		Named Menus
		Menu
		Tool Bar
			Display Settings
...
.enddata
.data CLASSPROPS
0000: 5461624368696C64 4E616D6573000800 73656C6563616F00 00
.enddata
.data CLASSPROPSSIZE
0000: 1900
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _sConsultaFrmOpcoesCampo = 'exec Prot..GrupoRecursoRestituicaoS @OrgaoCompetencia = 117100'
						! Set _sConsultaFrmOpcoesCampo = '
								SELECT 	DISTINCT
									CONVERT(VARCHAR(10), gr.OrgaoCompetencia),
									OA.Descricao
								FROM 	dbinfracao..OrgaoAutuante OA,
									Prot..GrupoRecurso gr
								WHERE	 gr.Restituicao = "S" and gr.OrgaoCompetencia = OA.Cod
								                 and OA.Cod = 117100 '
						!
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			Data Field: dfsDesOrgao
.data INHERITPROPS
0000: 0100
.enddata
.data CLASSPROPS
...
.enddata
				...
				Message Actions
					...
					On SAM_Create
						...
						Set _sConsultaDescrição = 'Exec OrgaoAutuanteS_desc_sint
										@Cod 	= :frmRelatorioRestTaxas.dfnOrgao'
						!
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			!
			Background Text: Órgão:
				Resource Id: 18240
				Class Child Ref Key: 0
				Class ChildKey: 0
				Class: cQCK1BackgroundText
...
.enddata
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Número do agente'
						!
						Set _bVisivel = TRUE
						!
						...
						!
						Set _sConsultaFrmOpcoesSQLOuSP = 'SP'
						Set _sConsultaFrmOpcoesCampo = 'exec Prot..GrupoRecursoPopularTaxas'
						! Set _sConsultaFrmOpcoesCampo = '
										SELECT 	DISTINCT
								CONVERT(VARCHAR(10), gr.Grupo), gr.Descricao
										FROM 	Prot..GrupoRecurso gr
										WHERE	gr.OrgaoCompetencia = :frmRelatorioRestTaxas.dfnOrgao
										and     gr.Restituicao = "S"
								                                 and    gr.Grupo in (6,7)'
						!
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
			Data Field: dfsDescGrupo
.data INHERITPROPS
0000: 0100
.enddata
.data CLASSPROPS
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						Set _bÉDependente = TRUE
						Set _bÉCampoDescrição = TRUE
						Set _sCamposResponsaveis[0] = 'dfnGrupo'
						Set _sConsultaDescrição = 'exec Prot..GrupoRecursoS_descricao @OrgaoCompetencia = :frmRelatorioRestTaxas.dfnOrgao,@Grupo = :frmRelatorioRestTaxas.dfnGrupo'
						!
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					On MSG_Habilita1
			!
			...
			Data Field: dfdInicial
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Data Incial do Intervalo da restituição.'
						!
						Set _bVisivel = TRUE
			...
			Data Field: dfdFinal
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Data finial do Intervalo da restituição.'
						!
						Set _bVisivel = TRUE
						Set _bSempreHabilitado = TRUE
						!
				...
				Message Actions
					...
					On SAM_Click
						...
						If SalIsNull( dfnOrgao )
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'Órgão de Competência deve ser informado', 'Atenção', MB_IconExclamation | MB_Ok )
							Return FALSE
						If SalIsNull( dfnGrupo )
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'Grupo de Restituição deve ser informado', 'Atenção', MB_IconExclamation | MB_Ok )
							Return FALSE
						Call SalWaitCursor( TRUE )
						Set sVariavel = ":frmRelatorioRestTaxas.sCodOrgao,:frmRelatorioRestTaxas.sDesOrgao,:frmRelatorioRestTaxas.sProcesso,:frmRelatorioRestTaxas.sSetor,:frmRelatorioRestTaxas.nDebito,
								:frmRelatorioRestTaxas.nParcela,:frmRelatorioRestTaxas.nBaixa,:frmRelatorioRestTaxas.nGuiaHab,:frmRelatorioRestTaxas.nTaxaHab,:frmRelatorioRestTaxas.sUsuario,
								:frmRelatorioRestTaxas.dData,:frmRelatorioRestTaxas.nValorRest,:frmRelatorioRestTaxas.nPagtoExercicio,:frmRelatorioRestTaxas.nFinanceiro,:frmRelatorioRestTaxas.sMotivo"
						Set sImput = 'Cod_Orgao,Des_Orgao,Processo,Setor,Debito,Parcela,Baixa,GuiaHab,TaxaHab,Usuario,DataCad,Val_Rest,PagtoExercicio,Financeiro,Motivo'
						If dfsSaida='I'
							Call SalReportPrint( hWndForm, 'rest_taxas.qrp', sVariavel , sImput, 1,RPT_PrintAll, 0, 0, nErro )
						Else
							If hWndReport != hWndNULL
								Call SalSendMsg( hWndReportView, SAM_Close, 0, 0)
							Set hWndReport= SalCreateWindow( frmReportView, hWndForm )
							Call SalSetWindowText( hWndReport, 'Relatório de restituição de taxas' )
							Call SalReportView( hWndForm, hWndReport,'rest_taxas.qrp', sVariavel, sImput, nErro )
...
.enddata
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Limpa a Tela'
						!
...
.enddata
				...
				Message Actions
					On SAM_Create
						Set _sTextoStatusBar = 'Fecha a Tela'
						!
		...
		Message Actions
			On SAM_Create
				Set _sTextoStatusBar = 'Relatório de restituicao de infração por órgão autuante'
				!
				Set sComandoImp = 'Exec	Prot..RestituicaoTaxasR
							@DataInicial		= :frmRelatorioRestTaxas.dfdInicial ,
							@DataFinal		= :frmRelatorioRestTaxas.dfdFinal,
							@OrgaoCompetencia 	= :frmRelatorioRestTaxas.dfnOrgao,
							@Grupo   		= :frmRelatorioRestTaxas.dfnGrupo '
				!
				Set sIntoImp= '
						:frmRelatorioRestTaxas.sCodOrgao,
						:frmRelatorioRestTaxas.sDesOrgao,
						:frmRelatorioRestTaxas.sProcesso,
						:frmRelatorioRestTaxas.sSetor,
						:frmRelatorioRestTaxas.nDebito,
						:frmRelatorioRestTaxas.nParcela,
						:frmRelatorioRestTaxas.nBaixa,
						:frmRelatorioRestTaxas.nGuiaHab,
						:frmRelatorioRestTaxas.nTaxaHab,
						:frmRelatorioRestTaxas.sUsuario,
						:frmRelatorioRestTaxas.dData,
						:frmRelatorioRestTaxas.nValorRest,
						:frmRelatorioRestTaxas.nPagtoExercicio,
						:frmRelatorioRestTaxas.nFinanceiro,
						:frmRelatorioRestTaxas.sMotivo'
				!
				Set _nEstadoInicialForm = 1
				Call SalCenterWindow( hWndForm )
				If hWndObjeto != hWndNULL
					Call SalDisableWindow( hWndObjeto )
				!
				Call SalSendClassMessage( SAM_Create, wParam, lParam )
			...
			On SAM_ReportNotify
				...
				If lParam = RPT_BeforePageHeader
					Call SalReportSetStringVar( hWndReport, 'Des_Sec', sGlobalSecretaria )
					Call SalReportSetStringVar( hWndReport, 'Des_Org', sGlobalDetran )
					Call SalReportSetNumberVar( hWndReport, 'Cod_Set', nGlobalSetor )
					Call SalReportSetStringVar( hWndReport, 'Des_Set', sGlobalSetor )
					Call SalReportSetStringVar( hWndReport, 'Cod_User', SqlUser )
					Call SalReportSetDateTimeVar( hWndReport, 'Dat_Ini', dfdInicial )
					Call SalReportSetDateTimeVar( hWndReport, 'Dat_Fin', dfdFinal )
			On SAM_ReportStart
			...
			On SAM_ReportFetchInit
				Call SybPrepareProc( hSql,sComandoImp,sIntoImp )
