...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmRelHistRestituicao
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Rel. Histórico de Pagamento de Restituição
		Icon File:
		...
		Description: Relatório de Histórico de Pagamento de Restituicao
		Named Menus
		Menu
		Tool Bar
			Display Settings
...
.enddata
				...
				Message Actions
					On SAM_Create
						...
						! Set _sConsultaFrmOpcoesCampo = 'select convert( varchar( 10), Cod), DescricaoSintetica from ' || DATABASE_INFRACAO || '..OrgaoAutuante'
						! Set _sConsultaFrmOpcoesCampo = '
								SELECT 	DISTINCT
									CONVERT(VARCHAR(10), GOA.OrgaoRestituicao),
									OA.DescricaoSintetica
								FROM 	dbinfracao..OrgaoAutuante OA,
									dbinfracao..GrupoOrgaoAutRest GOA
								WHERE	GOA.OrgaoRestituicao = OA.Cod '
...
.enddata
			...
			Pushbutton: pbImprimir
				...
				Message Actions
					On SAM_Create
						...
						Call SalSendClassMessage( SAM_Create, wParam, lParam )
					On SAM_Click
						If SalIsNull(dfnProtocolo) and SalIsNull(dfnFinanceiro)
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'O numero de protocolo ou financeiro é obrigatório.', 'Atenção', MB_IconExclamation | MB_Ok )
							Call SalSetFocus( dfnProtocolo )
							Return FALSE
						Call SalWaitCursor( TRUE )
						Set sVariavel = ':sProtocolo,:sNome,:sCgcCpf,:sSituacao,:nValorRest,:sFinanceiro,:sArquivo,:sListagem,:nExercicioPG,:dData,:nNumSeq'
						Set sImput =  'Protocolo,Nome,CgcCpf,Situacao,Valor,Financeiro,Arquivo,Listagem,Exercicio,Data,NumSeq'
						If dfsSaida='I'
							Call SalReportPrint( hWndForm, 'Rel_hist.qrp',sVariavel ,sImput , 1,RPT_PrintAll,0,0, nErro )
						Else
							If hWndReport != hWndNULL
								Call SalSendMsg( hWndReportView, SAM_Close, 0, 0)
							Set hWndReport= SalCreateWindow( frmReportView, hWndForm )
							Call SalSetWindowText( hWndReport, 'Rel. Histórico de Pagamento de Restituição' )
							Call SalReportView( hWndForm, hWndReport,'Rel_hist.qrp', sVariavel, sImput, nErro )
		...
		Message Actions
			On SAM_Create
				Set sComandoImp = '
						exec	Prot..RestitRel_HistoricoFinan
							@Prot		=	:frmRelHistRestituicao.dfnProtocolo ,
							@Financeiro	=	:frmRelHistRestituicao.dfnFinanceiro'
				Set sIntoImp= '
							:frmRelHistRestituicao.sProtocolo,
							:frmRelHistRestituicao.sNome,
							:frmRelHistRestituicao.sCgcCpf,
						                :frmRelHistRestituicao.sSituacao,
							:frmRelHistRestituicao.nValorRest,
							:frmRelHistRestituicao.sFinanceiro,
							:frmRelHistRestituicao.sArquivo,
							:frmRelHistRestituicao.sListagem,
						                :frmRelHistRestituicao.nExercicioPG,
						             	:frmRelHistRestituicao.dData,
							:frmRelHistRestituicao.nNumSeq'
				Set _nEstadoInicialForm = 1
				Set _sTextoStatusBar = 'Relatório de Historico de Restituicao '
