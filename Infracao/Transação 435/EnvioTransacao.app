...
.head 7 -  ! Set tblAutos._sComandoPesquisa = 'exec dbinfracao..RNFRegistroPontuacao'
...
tblAutos.colnInfracao,tblAutos.colnTransacao,'EV01',hSqlApp,'','','','',7,1,'3')
.head 8 -  Set nRow = nRow + 1
.head 7 -  ! Call SalDestroyWindow( mdiInfracao )
...
.head 8 -  Call SalTblSetRowFlags( tblAutos, ROW_Edited, nRow, TRUE )
.head 8 +  If tblAutos.colsTipoMovimento = 'E' 
.head 9 -  Call fTransmiteRENAINF(tblAutos.colsSerie,tblAutos.colnOrgaoAutuante,
tblAutos.colnAuto,
tblAutos.colnInfracao,tblAutos.colnTransacao,'EV01',hSqlApp,'','','','',7,2,'3')
.head 8 +  Else If tblAutos.colsTipoMovimento = 'M'
.head 9 -  Call fTransmiteRENAINF(tblAutos.colsSerie,tblAutos.colnOrgaoAutuante,
...
.head 7 -  ! Set frmEnviaNotificacaoAutuacao.dfsMensagem = 'COMUNICANDO COM BINIT.................'
.head 7 -  Set nRetorno = 20
.head 7 -  ! Recuperando o código da transação
.head 7 -  ! Set dfnTransacao = SalStrToNumber(SalStrMidX(sTringEnvio, 6, 3))
...
.head 8 +  ! If fMPSPrepareProc( hSqlComunicacao, 'exec ' || DATABASE_RENAVAM || '..TipoConsultaS @Transacao = :dfnTransacao', ':sTipoTransacao' )
...
.head 8 -  Set nRetorno = _FSocket32( sGlbIPRENAVAM, nGlbPortaRENAVAM, 20, sTringEnvio, 4096 )
.head 8 -  Set sTringEnvio = SalStrMidX( sTringEnvio, 55, 4096 )
.head 8 +  If nRetorno != 55 And nRetorno > 0
.head 9 -  Set sParametro1 = SalStrMidX(sTringEnvio,0,255)
.head 9 -  Set sParametro2 = SalStrMidX(sTringEnvio,255,255)
.head 9 -  Set sParametro3 = SalStrMidX(sTringEnvio,510,255)
.head 9 -  Set nNumseq = SalStrToNumber(SalStrMidX(sTringEnvio, 0, 6))
.head 9 -  Set nCodTransacao = SalStrToNumber(SalStrMidX(sTringEnvio,6,3))
.head 9 +  If fMPSPrepareProc( hSqlComunicacao, 'exec ' || DATABASE_RENAVAM || '..RetRenavamI
@Parametro1 = :sParametro1, @Parametro2 = :sParametro2, @Parametro3=:sParametro3', '' )
.head 10 -  Call SqlExecute( hSqlComunicacao )
.head 10 -  Set nRetorno = 0
.head 9 -  ! Set frmEnviaNotificacaoAutuacao.dfsMensagem = 'PROCESSANDO.....'
...
.head 12 -  ! Set frmEnviaNotificacaoAutuacao.dfsMensagem = 'PROCESSANDO.....'
.head 8 -  Call fMPSDisconnect( hSqlComunicacao )
.head 8 -  Return TRUE
.head 5 +  Function: fFuncaoComunicacao
.head 6 -  Description:
...
.head 7 -  Number: nRow
.head 6 +  Actions 
.head 7 -  Set nRegistros = 0
.head 7 -  Set nRow = 0
.head 7 -  Set bFimLeitura = FALSE
.head 7 -  ! Call SqlFetchNext( hSqlApp, nProximo )
.head 7 +  If nProximo = FETCH_EOF
.head 8 -  Call SalMessageBox( 'Não há notificações de multa a serem enviadas', 'Atenção', MB_IconExclamation | MB_Ok ) 
.head 8 -  Call SalWaitCursor( FALSE )
.head 8 -  Return FALSE
.head 7 -  ! Call SalTblInsertRow( tblAutos, nRow )
.head 7 -  ! Set tblAutos.colsArquivo = sArquivo
.head 7 +  Loop 
.head 8 -  Call SalSendMsg( dfmContador, SAM_Validate, nRegistros, 0)
.head 8 -  Set nRegistros = nRegistros + 1
.head 8 +  If NOT bFimLeitura
.head 9 +  If nTipoArquivo != nTipoArquivoAnt 
.head 10 +  If nTipoArquivoAnt != 0
.head 11 -  Call SalFileClose( hArquivo )
.head 10 -  Call SalTblInsertRow( tblAutos, nRow )
.head 10 -  Set tblAutos.colsArquivo = sArquivo
.head 10 -  Set nRow = nRow + 1
.head 10 -  Set nTipoArquivoAnt = nTipoArquivo
.head 10 +  If SalFileOpen( hArquivo, sArquivo, OF_Read | OF_Share_Exclusive)
.head 11 -  Call SalFileClose( hArquivo )
.head 11 -  Call SalFileCopy( sArquivo, SalStrLeftX( sArquivo,  SalStrLength( sArquivo ) - 3 ) || 'BCK', FALSE)
.head 10 +  If NOT SalFileOpen( hArquivo, sArquivo, OF_Write | OF_Create  )
.head 11 -  Call SalMessageBox( 'Não foi possível abrir Arquivo: '||sArquivo||' verifique se caminho está correto, ou se há espaço na unidade de disco ou ainda se existe permissão para a gravação do arquivo',
                                     'Atenção', MB_Ok|MB_IconStop )
.head 11 -  Return FALSE
.head 9 -  Call SalFilePutStr( hArquivo,
...
.head 10 -  Break 
.head 8 +  Else 
.head 9 -  Call SalFilePutStr( hArquivo, SalStrRepeatX( ' ', 790 ) )
.head 9 +  If SalNumberMod( nRegistros, 30 ) = 0
.head 10 -  Break 
