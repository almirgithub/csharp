...
0020: 0000000000FC0000 002C000000020000 0003000000FFFFFF FFFFFFFFFFF8FFFF
...
.enddata
	...
	Libraries
		...
		! File Include: frmRelCnhMov2.apl
		! File Include: frmRelCnhMovInf.apl
	Global Declarations
		Window Defaults
			Tool Bar
...
.enddata
		Default Classes
			...
			ActiveX:
		Application Actions
	Form Window: frmRelCnhMov
		Class: cQCK1Form
		Property Template:
		Class DLL Name:
		Title: Relatórios das impressões de CNH´s movimentadas/protocoladas por período 
		Icon File:
			...
			Pushbutton: pbImprimir
				...
				Message Actions
					...
					On SAM_Click
						...
						! Set sNomeQRP = 'RelCNHMovPeriodo.qrp'
						If SalIsNull( dfdInicial )
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'Data inicial deve ser escolhida !', 'Atenção', MB_Ok | MB_IconExclamation )
							Return FALSE
						If SalIsNull( dfdFinal )
							Call SalMessageBeep( 0 )
							Call SalMessageBox( 'Data final deve ser escolhida !', 'Atenção', MB_Ok | MB_IconExclamation )
							Return FALSE
						! Mostra o relatório
						If rbImpressora
							Call SalReportPrint( hWndForm, 'RelCNHMovPeriodo.qrp',  '
									sGlobalSecretaria, 			
									sGlobalDetran,
									nGlobalSetor,			
									sGlobalSetor,
									SqlUser,
									frmRelCnhMov.dDataImp,
									frmRelCnhMov.dfdInicial,
									frmRelCnhMov.dfdFinal,
									nProtocolo,
									sPlaca,
									sOrgaoAutuante,
									sSerie,
						...
						Else
							...
							Call SalSetWindowText( hWndReportView,  'Relatórios das impressões de CNH´s movimentadas/protocoladas por período ' )
							Call SalReportView( hWndForm, hWndReportView, 'RelCNHMovPeriodo.qrp', '
									sGlobalSecretaria, 			
									sGlobalDetran,
									nGlobalSetor,			
									sGlobalSetor,
...
.enddata
		...
		Window Variables
			...
			String: sCondutor
			String: sOficio
			String: sTesteAlcoolemia
			Number: nAuto
			Number: nDigito
			Number: nInfracao
			Number: nDesdobramento
			Number: nCNH
			Number: nProtocolo
			Date/Time: dDataOficio
			String: sObservacao
		Message Actions
			On SAM_Create
				Set _sTextoStatusBar = 'Tela de relatório de repasses.'
			...
			On SAM_ReportStart
				Call SalReportDlgOptions( SalNumberToWindowHandle( wParam ), 'Impressão', 'Imprimindo',
						'Relatórios das impressões de CNH´s movimentadas/protocoladas', 'por Período' )
				Return TRUE
			On SAM_ReportFetchInit
				Call SalWaitCursor( TRUE )
				! Call SalReportSetNumberVar( SalNumberToWindowHandle ( wParam ), 'nResultado', nResultado )
				!
				If fMPSPrepareProc( hSqlApp, 'exec dbinfracao..RelCNHRecolhidaS
								@DataInicial		= :frmRelCnhMov.dfdInicial,
								@DataFinal 		= :frmRelCnhMov.dfdFinal',
								':nProtocolo,
								:sPlaca,
								:sOrgaoAutuante,
								:sSerie,
				...
				Call SalWaitCursor( FALSE )
				!
				Return SqlGetResultSetCount( hSqlApp, nErro )
				If nErro = 0
					Call SalMessageBox('Não existe registros para essa pesquisa', 'Atenção', MB_IconExclamation | MB_Ok)
					Return FALSE
			On SAM_ReportFetchNext
				Call SqlFetchNext( hSqlApp, nErro )
				If nErro != FETCH_EOF
					Return TRUE
				Else
					Return FALSE
